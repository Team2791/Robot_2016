...
public class ShakerCamera implements Runnable {
    ...
    private static Image binaryFrame;
    ...
    private boolean modifyingCameraImage = false;
        ...
        while (true) {
            try {
                ...
                if (frame != null) {
                    ...
                    if (SmartDashboard.getBoolean("Debug Image") || AutoLineUpShot.isRunning()) {
                    ...
                    } else if (AutoLineUpShot.isRunning() || processImage) {
                        reading_particles.acquire();
                        measureAndGetParticles();
                        reading_particles.release();
                    } else {
                ...
                if (cameraAutoSettings && !cameraValsOnlyOnce) {
                    ...
                    modifyingCameraImage = true;
                    manualCamSettings();
                    cameraValsOnlyOnce = true;
                    modifyingCameraImage = false;
    ...
    private void manualCamSettings() {
        cam.setExposureAuto();
    ...
    private double getNormalizedCenterOfMass(double currentCenterInPixels) {
    ...
    public void processImage(boolean shouldProcess) {
        processImage = shouldProcess;
    }
    ...
    public ParticleReport getTarget() {
        ...
        try {
            reading_particles.acquire();
            reports = (ArrayList<ParticleReport>) particles.clone();
            reading_particles.release();
        } catch (InterruptedException e) {
