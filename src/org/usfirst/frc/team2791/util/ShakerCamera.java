...
public class ShakerCamera implements Runnable {
	...
	public double getRange() {
		...
		double range = (TARGET_HT_INCHES - CAMERA_HT_INCHES) / Math.tan(
				getNormalizedCenterOfMass(getTarget().CenterOfMassY) * (CAMERA_WIDTH_DEGREES / 2) + CAMERA_PITCH_DEG)
				* (Math.PI / 180);
	...
	public ParticleReport getTarget() {
		...
		NIVision.imaqDrawShapeOnImage(binaryFrame, binaryFrame, r, NIVision.DrawMode.PAINT_VALUE,
				NIVision.ShapeMode.SHAPE_RECT, 10f);// highlight the choosen
