...
public class NewShakerCamera {
    ...
    private NIVision.Range TARGET_HUE_RANGE = new NIVision.Range(0, 255); //Hue range
    private NIVision.Range TARGET_SAT_RANGE = new NIVision.Range(0, 255); //Saturation range
    private NIVision.Range TARGET_VAL_RANGE = new NIVision.Range(0, 255);//value range
    private double AREA_MINIMUM = 0.5; //Min area (probably best if gotten from practice field or target)
    private double WID_RATIO = 2.22; //Tote long side = 26.9 / Tote height = 12.1 = 2.22
    //    private double HEIGHT_RATIO = 1.4; //Tote short side = 16.9 / Tote height = 12.1 = 1.4
    ...
    public NewShakerCamera(String camPort) {
        ...
        session = NIVision.IMAQdxOpenCamera(camPort, NIVision.IMAQdxCameraControlMode.CameraControlModeController);
    ...
    public void update(boolean doTargetting, boolean displayTargettingToDash) {//displaying targetting is very SLOWWW!!!
        NIVision.IMAQdxStartAcquisition(session);//configure grab for grabbing from webcam
        NIVision.IMAQdxGrab(session, frame, 1);//grab frame from camera
        if (doTargetting) {//if targetting should be done, this is here so we wont always have targetting on
            NIVision.imaqColorThreshold(binaryFrame, frame, 255, NIVision.ColorMode.HSV, TARGET_HUE_RANGE, TARGET_SAT_RANGE,
                    TARGET_VAL_RANGE);//color threshold, returns a binary image(black and white), according to ranges
            ...
            int numParticles = NIVision.imaqCountParticles(particleBinaryFrame, 1);//finds all the particles
            System.out.println("Filtered particles" + numParticles);
            if (numParticles > 0) {
                //Measure particles and sort by particle size
                String output = "";
                for (int particleIndex = 0; particleIndex < numParticles; particleIndex++) {
                    //iterates through each particle ... in future should remove particle if criteria not met
                    ...
                    NIVision.Rect r = new NIVision.Rect((int) par.BoundingRectTop, (int) par.BoundingRectLeft,
                            Math.abs((int) (par.BoundingRectTop - par.BoundingRectBottom)), Math.abs((int)
