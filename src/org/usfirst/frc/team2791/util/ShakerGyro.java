...
public class ShakerGyro extends SensorBase implements Runnable {
    ...
    private byte FIRST_BYTE_DATA = 0x3; //mask to find sensor data bits on first byte: X X X X X X D D
    private char THIRD_BYTE_DATA = 0xFC; //mask to find sensor data bits on third byte: D D D D D D X X
    private SPI m_spi;
    ...
    int assemble_sensor_data(byte[] data) {
        ...
        result |= (data[2] & THIRD_BYTE_DATA) >> 2; // remove last 2 bits keep first 6
