...
public class TeleopHelper extends ShakerHelper {
	...
	private void operatorRun() {
		...
		if (intake.getIntakeState().equals(ShakerIntake.IntakeState.EXTENDED)) {// checks
																				// if
																				// the
																				// intake
																				// is
...
//		String firstCamera = "cam1";
//		String secondCamera = "cam2";
//		Boolean draw;
//		if (operatorJoystick.getButtonSt() || driverJoystick.getButtonSt() || isShooterNotLow)
//			// this will switch cameras according to the state of the shooter
//			// and according to the driver or operator choice
//			cam.switchCameras(secondCamera);
//
//		else if (operatorJoystick.getButtonSt() && isShooterNotLow || driverJoystick.getButtonSt() && isShooterNotLow)
//			// if the shooter is not in the low position and driver/op hits
//			// start it goes back to the other camera
//			cam.switchCameras(firstCamera);
//		else
//			cam.switchCameras(firstCamera);
//		;
//		cam.update(true);

		if (operatorJoystick.getButtonSel()) {// If the operator needs to
												// override the driver for some
												// reason they can hold the
